...
def WGCNA_shiny_appname_generator():
    output_app_dir_name = "wgcna_" + str(random.randint(1000000000, 9999999999))
    output_app_dir = '/data1/Shiny_Apps/' + output_app_dir_name
    return output_app_dir, output_app_dir_name


def WGCNA_shiny_app_creator(wgcna_expression_df, wgcna_trait_df, output_app_dir, output_app_dir_name):
...
shinyApp(ui,server)
    ...
    try:
        with open(output_app_dir + '/app.R', mode='w') as out_fh:
            out_fh.write(wgcna_mainapp_text_raw)
    except IOError:
        return "IO error occurred while writing shiny app."
