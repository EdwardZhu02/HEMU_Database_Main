...
def gokegg_enrich_plt(_gokegg_df):

    filename_id = str(random.randint(1000000000, 9999999999))
    # For background gene profile generation
    bg_annot = "Mainapp/R_visualization/gokegg/zea.emapper.annotations"

    robjects.globalenv['gokegg_dataframe'] = _gokegg_df
    robjects.globalenv['filename_id'] = filename_id
    robjects.globalenv['bg_annot'] = bg_annot

    plt_rscript = '''

gokegg_plotter <- function (gokegg_dataframe, filename_id, bg_annot){
    ...
    ko_pathway_name = read.table("Mainapp/R_visualization/gokegg/KEGG_pathway_ko_uniq.txt", header=T, sep="\t")
    ...
    robjects.r['gokegg_plotter'](_gokegg_df, filename_id, bg_annot)  # Execute function
